...
ctx_model = DPRContextEncoder.from_pretrained('/FaceDPRctx', local_files_only=True)
ctx_tokenizer = DPRContextEncoderTokenizer.from_pretrained('/FaceDPRctx',local_files_only=True)

question_model = DPRQuestionEncoder.from_pretrained('/FaceDPRQ', local_files_only=True)
question_tokenizer = DPRQuestionEncoderTokenizer.from_pretrained('/FaceDPRQ', local_files_only=True)
...
contexts2 = [str1, str2, str3, str4, str5, str6, str7, str8, str9, str10, str11, str12, str13, str14, str15, str16,
             str17,str19,str20, str22,str23,str24,str25]
# 10 is too long
...
tokenizerp = AutoTokenizer.from_pretrained('/BERTSQ', local_files_only=True)
modelp = AutoModelForQuestionAnswering.from_pretrained('/BERTSQ', local_files_only=True)
...
for j in range(1):
    ...
    for i, xq_vec in enumerate(xq.pooler_output):
        probs = cos_sim(xq_vec, xb.pooler_output)   #similarity by cos-sim , others?
        ...
        [print(ans,"\n", "-----------") for index, ans in enumerate(answer_map["answers"]) if len(ans) > 0]
